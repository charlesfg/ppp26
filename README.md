# 


# 🚀 PPP 2026 - Programação em C - Materiais e Recursos

Bem-vindo ao repositório da disciplina de **PPP 2026 - Prof. Charles**! Aqui você encontrará materiais de apoio, exemplos de código, exercícios e links úteis para auxiliar no seu aprendizado.


## 📌 Estrutura do Repositório

Este repositório está organizado da seguinte forma:

📂 **/exercicios** – Lista de exercícios propostos para prática.  
📂 **/exemplos** – Códigos de exemplo explicados durante as aulas.  
📂 **/slides** – Apresentações das aulas.  

---

## 📚 Materiais de Apoio

🔗 **Slides das Aulas:**  
Acesse as apresentações utilizadas durante as aulas na [`/slides`](slides.md).  

🔗 **Lista de Exercícios:**  
Pratique sua programação resolvendo os exercícios propostos em [`/exercicios`](./exercicios).  

🔗 **Soluções:**  
Confira os códigos demonstrados em aulas e seus comentários explicativos em [`/src`](./src).  


# **Material de Apoio:**  
- [The C Programming Language - K&R](https://www.amazon.com.br/C-Programming-Language-2nd/dp/0131103628)  
- [cppreference.com - C Language Reference](https://en.cppreference.com/w/c)  
- [Plataforma para aprendizado de C  - Exercism](https://exercism.org/tracks/c/exercises)  
- [Referência Rápida para funções](https://devdocs.io/c/)
- [Visualizador de Código](https://pythontutor.com/c.html#mode=edit)


---

## 💻 Ambiente de Desenvolvimento

Na disciplina iremos utilizar os seguinte ambiente de programação C:

🛠 **Compilador:**  
- GCC: `gcc` (Linux/macOS/Wsl)  
- Clang: (macOS - ARM)[LLVM Clang](https://clang.llvm.org/)  

🖥 **IDEs e Editores:**  
- [VS Code](https://code.visualstudio.com/) + extensão C/C++   

🛠 **Utilizando o Makefile para automatizar a compilação:**  
- [Introdução ao Makefile](introMake.md)

---

📝 Bons estudos e boas práticas de programação! 🚀  
