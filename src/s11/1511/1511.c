...
int escrever_binario(const char* nome_ficheiro, struct list_node* lista) {
    ...
    /* Skips header */
    struct list_node* atual = lista->next;
...
int escrever_texto(const char* nome_ficheiro, struct list_node* lista) {
    ...
    while (atual != NULL) {
        size_t escritos = fprintf(f, "Pessoa: %s\nidade: %d\n",
                                  atual->per.name, atual->per.age);
...
int main(void) {
    ...
    if (escrever_texto("pessoas.txt", lista)) {
        printf("\nFicheiro pessoas.txt escrito com sucesso.\n");
    } else {
        printf("\nErro ao escrever o ficheiro.\n");
    }
